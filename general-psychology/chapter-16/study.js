// TRSS — TruReview Student Studier: Chapter Study & Practice Reference
// Standalone chapter page. Chapter identity comes from window.TRSS_CHAPTER
// (set by the page before this script loads).
(function() {
  "use strict";
  ...
  const extraCards = (window[TRSS_CHAPTER.prefix + "_EXTRA_CARDS"] || []).map(c => ({ ...c, sourceSet: "research-skill" }));
  ...
  const CHAPTER_ID = "chapter-" + TRSS_CHAPTER.num;
  ...
  const localStore = {
    loadProgress() {
      try {
        const raw = localStorage.getItem(PROGRESS_STORAGE_KEY) || localStorage.getItem("trss-chapter" + TRSS_CHAPTER.num + "-progress");
    ...
    resetAll() {
      try {
        ...
        localStorage.removeItem("trss-chapter" + TRSS_CHAPTER.num + "-progress");
      } catch (e) {}
    }
  };
  ...
  function handleStudyBack(e) {
    e.preventDefault();
    window.location.href = "../../#/" + TRSS_CHAPTER.course;
  }
  ...
  // Test hook: exposes quiz builders to the Node quiz-logic test only.
  // Fires only when the harness defines window.__TRSS_TEST_HOOK__.
  if (typeof window !== "undefined" &&
      typeof window.__TRSS_TEST_HOOK__ === "function") {
